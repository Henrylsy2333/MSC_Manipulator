...
void CSample2AxesControl_MFCDlg::OnBnClickedButtonStart()
{
	...
	ret = NEC_StartDriver();
	if( ret != 0 ) { MessageBox("NEC_StarDriver failed!"); CDialogEx::OnCancel(); return; }

	ret = NEC_GetRtMasterId( &masterId );
	if( ret != 0 ) { MessageBox( "NEC_GetRtMasterId failed!" ); CDialogEx::OnCancel(); return; }

	ret = NEC_ResetEcMaster( masterId );
	if( ret != 0 ) { MessageBox( "NEC_ResetEcMaster failed!" ); CDialogEx::OnCancel(); return; }

	ret = NEC_LoadNetworkConfig( masterId, "C:\\ENI_NexCAT_Export.xml", START_NETWORK_OPT_MASK_NIC_PORT);
	if( ret != 0 ) { MessageBox( "NEC_StartNetworkEx failed!" ); CDialogEx::OnCancel(); return; }

	ret = NEC_LoadRtxApp( "C:\\Program Files\\NEXCOM\\NexECMRtx\\Samples\\RTX2012\\Sample_2Layer\\VS2010\\Sample2AxesControl_MFC\\RTSSDebug\\RtxApp.rtss" );
	if( ret != 0 ) { MessageBox("NEC_LoadRtxApp RTX_app failed!"); CDialogEx::OnCancel(); return; }
